...
Приложение настроено на работу с уже существующей таблицей
[«Список техники»](https://docs.google.com/spreadsheets/d/1oVQnAif57GhY0ScIFjylCxyL6bbII1TLc1bopGhHVUA/edit)
(лист с данными называется именно так — это задано в `CONFIG.SHEET_NAME` в
`Config.gs`). **Новый лист скрипт не создаёт** — он читает и пишет в
существующие столбцы по индексу, не трогая ничего, что не входит в модель
приложения (включая легаси-дубли столбцов).
