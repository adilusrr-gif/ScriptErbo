...
4. В левой панели редактора откройте **Services** (значок **+**) и добавьте
   **Google Sheets API**. Без этого `getAll()` не сможет узнать, какие
   строки скрыты фильтром в таблице, и будет падать с ошибкой 403.
